...
const SCORE_META = {
  strong:  { label: 'Strong fit',  color: '#4ade80', bg: 'rgba(74,222,128,0.08)',  border: 'rgba(74,222,128,0.2)' },
  good:    { label: 'Good fit',    color: '#38bdf8', bg: 'rgba(56,189,248,0.08)',  border: 'rgba(56,189,248,0.2)' },
  partial: { label: 'Partial fit', color: '#fbbf24', bg: 'rgba(251,191,36,0.08)', border: 'rgba(251,191,36,0.2)' },
  weak:    { label: 'Weak fit',    color: '#fb923c', bg: 'rgba(251,146,60,0.08)', border: 'rgba(251,146,60,0.2)' },
};

function AiEvalBlock({ aiEval }) {
  const meta = SCORE_META[aiEval.score] || SCORE_META.partial;
  return (
    <div style={{ marginTop: 8, padding: '8px 10px', background: meta.bg, border: `1px solid ${meta.border}`, borderRadius: 6 }}>
      <span style={{ color: meta.color, fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em', marginRight: 8 }}>
        {meta.label}
      </span>
      <span style={{ fontSize: 12, color: 'var(--text2)', lineHeight: 1.5 }}>{aiEval.summary}</span>
    </div>
  );
}
...
export default function JobCard({ job, onStatusChange, onEdit, onMarkApplied, onEvaluate, evaluating }) {
  ...
  const canEvaluate = job.status === STATUSES.EVALUATE && Boolean(job.jobDescription || job.applicationUrl);
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {job.aiEval && <AiEvalBlock aiEval={job.aiEval} />}
        </div>
        ...
        {isActive && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end', flexShrink: 0 }}>
            {canEvaluate && onEvaluate && (
              <button
                disabled={evaluating}
                onClick={() => !evaluating && onEvaluate(job)}
                style={{
                  ...btn,
                  color: evaluating ? '#64748b' : '#a78bfa',
                  borderColor: evaluating ? 'rgba(255,255,255,0.08)' : 'rgba(167,139,250,0.3)',
                  cursor: evaluating ? 'not-allowed' : 'pointer',
                }}
                onMouseEnter={e => { if (!evaluating) { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.color = '#f1f5f9'; } }}
                onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = evaluating ? '#64748b' : '#a78bfa'; }}
              >
                {evaluating ? (
                  <>
                  ...
                  <>
                    ...
                    {job.aiEval ? 'Re-evaluate' : 'Evaluate'}
                  </>
                )}
              </button>
            )}
